  ...
  return (
    <div className="space-y-6">
      <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 text-center">
        ...
        <h2 className="text-2xl font-bold text-slate-900 mb-2">Find Nearby Hospitals</h2>
        ...
        {!location ? (
          ...
          <div className="flex items-center justify-center gap-2 text-emerald-600 font-medium">
            ...
            <button onClick={requestLocation} className="ml-2 text-xs text-blue-600 hover:underline">Refresh</button>
          </div>
        )}
      </div>

      {error && (
        <div className="bg-red-50 border border-red-100 p-4 rounded-xl text-red-600 text-sm flex items-center gap-3">
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div className="w-12 h-12 bg-blue-50 rounded-xl flex items-center justify-center group-hover:bg-blue-600 transition-colors">
                  <Hospital className="w-6 h-6 text-blue-600 group-hover:text-white transition-colors" />
                </div>
                <a 
              ...
              <p className="text-sm text-slate-500 mb-4 line-clamp-2">{hospital.address}</p>
              <a 
                href={hospital.mapsUrl} 
                target="_blank" 
                rel="noopener noreferrer"
                className="w-full bg-slate-50 hover:bg-blue-50 text-slate-600 hover:text-blue-600 font-semibold py-2 rounded-lg transition-all flex items-center justify-center gap-2 text-sm"
              >
      ...
      {isLoading && hospitals.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 gap-4">
          ...
          <p className="text-slate-500 font-medium">Searching for hospitals nearby...</p>
        </div>
      )}
    </div>
  );
